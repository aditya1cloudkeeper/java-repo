#!/usr/bin/env bash
# Recommendation : GCS Storage Class Tier Optimization v2 (Overprovisioned)
# Source doc     : Overprovisioned __ GCS_Storage_Class_Tier_Optimization_Recommendation.md
#
# Difference from validate_overprovisioned_gcs_storage_class.sh
# ------------------------------------------------------------
#   v1 emitted nothing for projects whose buckets are almost all legacy-class,
#   because IGNORED_STORAGE_CLASSES excluded REGIONAL and MULTI_REGIONAL as well
#   as ARCHIVE. Those two are legacy names for Standard, not colder tiers, so
#   excluding them discarded 704 of 726 buckets in rl-global-eu-development and
#   82 of 84 in rl-global-us-test. v2 excludes ARCHIVE only.
#
#   Reaching ARCHIVE also needs -d 365: idleDays is capped by the lookback
#   window, so the default -d 30 can only ever emit NEARLINE. To reproduce the
#   dashboard's "not accessed for more than 365+ days -> ARCHIVE" rows, run:
#
#       validate_overprovisioned_gcs_storage_class_v2.sh -p PROJECT -d 365
#
#   Caveat carried over from v1: the emitted action describes changing the
#   bucket's default storage class, which does NOT retier objects that already
#   exist in the bucket. Realising the saving needs an Object Lifecycle
#   Management rule with a SetStorageClass action, or a rewrite of the objects.
#   The wording is kept as-is for parity with the dashboard.
#
# Detection logic:
...
# Excluded as a source class: ARCHIVE only, because it is already the coldest
# tier and has nothing to be upgraded to.
#
# v1 also excluded MULTI_REGIONAL and REGIONAL as "legacy classes the
# recommendation does not act on". That was wrong. Per
# cloud.google.com/storage/docs/storage-classes both are legacy *names* for
# Standard, not colder tiers: Regional storage is equivalent to Standard but
# restricted to regions, and Multi-Regional is equivalent to Standard for
# multi- and dual-regions. They carry Standard pricing and can be tiered to
# NEARLINE/COLDLINE/ARCHIVE exactly like STANDARD.
#
# Excluding them removed the overwhelming majority of real candidates: 704 of
# 726 buckets in rl-global-eu-development and 82 of 84 in rl-global-us-test.
# STORAGE_CLASS_RANK already maps both to rank 0 alongside STANDARD, and
# upgrade_only() still blocks any sideways or backwards move, so admitting them
# here is sufficient and safe.
IGNORED_STORAGE_CLASSES = frozenset(("ARCHIVE",))
